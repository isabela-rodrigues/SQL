...
desconto_total AS (
    ...
    FROM salario
    LEFT JOIN emp_desc ON emp_desc.matr = salario.matr
    LEFT JOIN desconto ON emp_desc.cod_desc = desconto.cod_desc
    GROUP BY salario.matr, salario.nome, salario.lotacao_div, salario.salario_total
),
geral AS (
    SELECT a.*,
...
FROM geral
LEFT JOIN media ON geral.cod_dep = media.cod_dep
